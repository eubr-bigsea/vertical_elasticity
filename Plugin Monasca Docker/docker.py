...
class Docker(checks.AgentCheck):
    ...
    def check(self, instance):
        ...
        try:
            ...
            msg ="Could not get containers from Docker API " + docker_url + " version: " + self.docker_version + "skipping Docker check - {}"
    ...
    def _report_container_metrics(self, container_dict, add_kubernetes_dimensions, dimensions):
        ...
        for container in container_dict.itervalues():
            #self.log.warning("serlophug - container['Id']: " + container['Id'])
            try:
    ...
    def _report_cgroup_cpuacct(self, container_id, container_dimensions):
        ...
        # Edit by serlophug
        dirpath = MYCGROUPS_PATH + 'cpuacct/' + container_id
        #self.log.warning('serlophug - dirpath: ' + dirpath)
        if os.path.isdir(dirpath):
            stat_file = dirpath + '/cpuacct.stat'
        # End edit
        stats = self._parse_cgroup_pairs(stat_file)
        #if os.path.isdir(dirpath):
        #    self.log.warning("serlophug -  stats['user']: " + str(stats['user']) + ", stats['system']: " + str(stats['system']) )
        #self.log.warning(str(stats))
    ...
    def _report_cgroup_blkio(self, container_id, container_dimensions):
        stat_file = self._get_cgroup_file('blkio', container_id, 'blkio.throttle.io_service_bytes')
